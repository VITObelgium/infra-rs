...
use crate::{arrowraster::ArrowRasterNum, rasterio, ArrowRaster, Error, GeoMetadata, Raster};
...
pub fn read_arrow_raster<T: ArrowRasterNum<T> + GdalType>(path: &std::path::Path) -> Result<ArrowRaster<T>, Error>
where
...
pub fn read_dense_raster_with_extent<T: ArrowRasterNum<T> + GdalType>(path: &std::path::Path, extent: &GeoMetadata) -> Result<ArrowRaster<T>, Error>
where
    T::TArrow: ArrowPrimitiveType<Native = T>,
{
    use gdal::Dataset;
...
mod tests {
    ...
    fn test_read_dense_raster_as_float() {
        ...
        let ras = read_arrow_raster::<f32>(path.as_path()).unwrap();
