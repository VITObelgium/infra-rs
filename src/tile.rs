use crate::{coordinate::Coordinate, crs, geoconstants::EARTH_CIRCUMFERENCE_M, geometadata::GeoMetadata, latlonbounds::LatLonBounds, rect::Rect, Point};
...
pub fn tile_index(tile: &Tile, meta: &GeoMetadata) -> Option<i32> {
    ...
    if meta.is_point_on_map(upper_left) {
        let tiles_per_row = (meta.columns() as f64 / TILE_SIZE as f64) as i32;

        let mut cell = meta.point_to_cell(upper_left);
        cell.row /= TILE_SIZE as i32;
        cell.col /= TILE_SIZE as i32;
...
pub fn pixel_size_at_zoom_level(zoom_level: i32) -> f64 {
    ...
    meters_per_tile / TILE_SIZE as f64
}
...
mod tests {
    ...
    fn test_pixel_size_at_zoom_level() {
        ...
        assert_relative_eq!(pixel_size_at_zoom_level(0), 156543.03, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(1), 78271.52, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(2), 39135.76, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(3), 19567.88, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(4), 9783.94, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(5), 4891.97, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(6), 2445.98, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(7), 1222.99, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(8), 611.50, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(9), 305.75, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(10), 152.87, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(11), 76.437, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(12), 38.219, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(13), 19.109, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(14), 9.5546, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(15), 4.7773, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(16), 2.3887, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(17), 1.1943, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(18), 0.5972, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(19), 0.298, epsilon = EPS);
        assert_relative_eq!(pixel_size_at_zoom_level(20), 0.149, epsilon = EPS);
    }

    #[test]
    fn calculate_zoom_level() {
        assert_eq!(zoom_level_for_pixel_size(10.0, true), 14);
        assert_eq!(zoom_level_for_pixel_size(100.0, true), 11);

        assert_eq!(zoom_level_for_pixel_size(10.0, false), 13);
        assert_eq!(zoom_level_for_pixel_size(100.0, false), 10);
    }
}
