use crate::{arrayops, ArrayMetadata, DenseArray, RasterNum};
...
macro_rules! dense_raster_op {
    (   $op_trait:path, // name of the trait e.g. std::ops::Add
        ...
        impl<T: RasterNum<T>, Metadata: ArrayMetadata> $op_trait for DenseArray<T, Metadata> {
        ...
        impl<T: RasterNum<T>, Metadata: ArrayMetadata> $op_assign_trait for DenseArray<T, Metadata> {
        ...
        impl<T: RasterNum<T>, Metadata: ArrayMetadata> $op_assign_scalar_trait for DenseArray<T, Metadata> {
        ...
        impl<T: RasterNum<T>, Metadata: ArrayMetadata> $op_assign_ref_trait for DenseArray<T, Metadata> {
            fn $op_assign_fn(&mut self, other: &DenseArray<T, Metadata>) {
                self.binary_inplace(&other, |x, y| {
                    x.$op_assign_nodata_fn(y);
                });
        ...
        impl<T: RasterNum<T>, Metadata: ArrayMetadata> $scalar_op_trait for DenseArray<T, Metadata> {
            type Output = DenseArray<T, Metadata>;
