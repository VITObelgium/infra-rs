...
impl CogAccessor {
    ...
    pub fn read_tile_data(&self, tile_source: &TileSource, mut reader: impl Read + Seek) -> Result<AnyDenseArray> {
        Ok(match self.meta.data_type {
            ArrayDataType::Uint8 => AnyDenseArray::U8(self.read_tile_data_as::<u8>(tile_source, &mut reader)?),
            ArrayDataType::Uint16 => AnyDenseArray::U16(self.read_tile_data_as::<u16>(tile_source, &mut reader)?),
            ArrayDataType::Uint32 => AnyDenseArray::U32(self.read_tile_data_as::<u32>(tile_source, &mut reader)?),
            ArrayDataType::Uint64 => AnyDenseArray::U64(self.read_tile_data_as::<u64>(tile_source, &mut reader)?),
            ArrayDataType::Int8 => AnyDenseArray::I8(self.read_tile_data_as::<i8>(tile_source, &mut reader)?),
            ArrayDataType::Int16 => AnyDenseArray::I16(self.read_tile_data_as::<i16>(tile_source, &mut reader)?),
            ArrayDataType::Int32 => AnyDenseArray::I32(self.read_tile_data_as::<i32>(tile_source, &mut reader)?),
            ArrayDataType::Int64 => AnyDenseArray::I64(self.read_tile_data_as::<i64>(tile_source, &mut reader)?),
            ArrayDataType::Float32 => AnyDenseArray::F32(self.read_tile_data_as::<f32>(tile_source, &mut reader)?),
            ArrayDataType::Float64 => AnyDenseArray::F64(self.read_tile_data_as::<f64>(tile_source, &mut reader)?),
        })
    }
    ...
    pub fn parse_tile_data_as<T: ArrayNum + HorizontalUnpredictable>(
        ...
        let tile_data = match tile_source {
            TileSource::Aligned(tile_meta) => io::parse_tile_data(
                tile_meta,
                self.meta.tile_size,
                self.meta.geo_reference.nodata(),
                self.meta.compression,
    ...
    fn cog_metadata() -> Result<()> {
        ...
        for pyramid in cog.metadata().pyramids.iter() {
            ...
            for tile in &pyramid.tile_locations {
                let tile_data = cog.read_tile_data(&TileSource::Aligned(*tile), &mut reader)?;
                ...
                let tile_data = cog.read_tile_data_as::<u8>(&TileSource::Aligned(*tile), &mut reader)?;
    ...
    fn compare_compression_results() -> Result<()> {
            ...
            .zip(cog_lzw_compression.metadata().pyramids.iter())
        {
            ...
            for (tile, tile_lzw) in pyramid_no_compression.tile_locations.iter().zip(pyramid_lzw.tile_locations.iter()) {
                let tile = TileSource::Aligned(*tile);
                let tile_lzw = TileSource::Aligned(*tile_lzw);
