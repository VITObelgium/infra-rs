...
mod bench {
    ...
    pub fn bench_colormap<T: num::Num + num::NumCast + Copy>(c: &mut Criterion) {
        let raster_size = RASTER_HEIGHT * RASTER_WIDTH;
        let data = allocate::aligned_vec_from_iter((0..raster_size).map(|i| -> T { NumCast::from(i).unwrap() }));
...
#[cfg(not(target_arch = "wasm32"))]
criterion::criterion_main!(bench::cmap_benches_f32);

#[cfg(target_arch = "wasm32")]
fn main() {
    println!("Benchmarks are not supported on wasm32.");
}
