...
impl CogTileProvider {
    pub fn new(path: &Path, _opts: &TileProviderOptions) -> Result<Self> {
        let cog = WebTilesReader::new(GeoTiffMetadata::from_file(path)?)?;
        ...
        let meta = LayerMetadata {
            ...
            tile_size: Some(meta.chunk_row_length()),
...
mod tests {
    ...
    fn create_test_cog(input_tif: &Path, output_tif: &Path, tile_size: u32, compress: &str) -> Result<()> {
        ...
        geo::raster::algo::gdal::warp_to_disk_cli(&src_ds, output_tif, &options, &vec![], None).expect("Failed to create test COG file");
