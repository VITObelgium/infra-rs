...
use inf::{
    ...
    rasterio::{self, guess_raster_format_from_filename, RasterFormat},
...
fn read_pixel_from_file(raster_path: &Path, coord: Point<f64>) -> Result<Option<f32>> {
    let ds = rasterio::open_raster_read_only(raster_path)?;
    let mut meta = rasterio::metadata_from_dataset_band(&ds, 1)?;
    ...
    rasterio::data_from_dataset_with_extent(&ds, &meta, 1, &mut data)?;
...
pub fn raster_pixel(raster_path: &Path, mut coord: Coordinate, layer_name: Option<&str>) -> Result<Option<f32>> {
    ...
    let meta = rasterio::metadata_from_file_with_options(raster_path, &open_opt)?;
