...
impl MbtilesTileProvider {
    pub fn new(db_path: &Path) -> Result<Self> {
        ...
        let mut meta = LayerMetadata {
            ...
            nodata: None,
            min_value: meta_map.remove("min_value").and_then(|s| f32::from_str(&s).ok()).unwrap_or(0.0),
            max_value: meta_map.remove("max_value").and_then(|s| f32::from_str(&s).ok()).unwrap_or(0.0),
